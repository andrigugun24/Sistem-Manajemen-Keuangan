...
export default function KategoriKeuanganEdit({ kategori_keuangan }) {
    const { data, setData, put, processing, errors } = useForm({
        nama_kategori: kategori_keuangan.nama_kategori || '',
        jenis: kategori_keuangan.jenis || 'Pemasukan',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('kategori-keuangan.update', kategori_keuangan.id));
    };

    return (
        <div className="space-y-6 lg:space-y-8 font-display antialiased pb-10">
            <Head title="Edit Kategori Keuangan" />
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mt-2">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h1 className="text-[28px] font-bold text-slate-900 dark:text-white tracking-tight">Edit Kategori Keuangan</h1>
                    </div>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm md:text-base ml-7">Perbarui penamaan sumber pendanaan (masuk) atau pos beban pendanaan (keluar) yayasan.</p>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[20px] border border-slate-200/60 dark:border-slate-800 shadow-sm overflow-hidden p-6 md:p-8 max-w-3xl">
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Jenis Aliran Dana - Disabled for Edit typically, but if enabled, here is the UI */}
                        <div className="space-y-2 md:col-span-2 border-b border-slate-100 dark:border-slate-800 pb-6 mb-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <label className={`
                                    relative flex items-center p-4 cursor-pointer rounded-xl border-2 transition-all opacity-70 pointer-events-none
                            ...
                            <p className="text-xs text-amber-600 mt-2 font-medium">ⓘ Jenis kategori keuangan tidak dapat diubah setelah dibuat. Silakan buat kategori baru jika salah memilih jenis.</p>
                        </div>
...
KategoriKeuanganEdit.layout = page => (
