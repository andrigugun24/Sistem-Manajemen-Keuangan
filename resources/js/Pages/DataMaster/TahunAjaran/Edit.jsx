...
export default function TahunAjaranEdit({ tahun_ajaran }) {
    const { data, setData, put, processing, errors } = useForm({
        nama_tahun_ajaran: tahun_ajaran.nama_tahun_ajaran || '',
        semester: tahun_ajaran.semester || 'Ganjil',
        aktif: tahun_ajaran.aktif || false,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('tahun-ajaran.update', tahun_ajaran.id));
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mt-2">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h1 className="text-[28px] font-bold text-slate-900 dark:text-white tracking-tight">Edit Tahun Ajaran</h1>
                    </div>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm md:text-base ml-7">Perbarui periode akademik dan status semester aktif.</p>
                </div>
            </div>
