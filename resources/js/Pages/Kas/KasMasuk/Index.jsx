...
export default function KasMasukIndex() {
    const kasMasukData = [
        { id: 1, tanggal: '20/02/2026', kode: 'KM-001', kategori: 'Dana BOS', sumber: 'Kemendikbud', nominal: 85000000, keterangan: 'Dana BOS Triwulan 1', status: 'verified' },
        { id: 2, tanggal: '15/02/2026', kode: 'KM-002', kategori: 'SPP', sumber: 'Siswa (Batch)', nominal: 10500000, keterangan: 'Rekap SPP Minggu ke-3', status: 'verified' },
        { id: 3, tanggal: '10/02/2026', kode: 'KM-003', kategori: 'Donasi', sumber: 'PT ABC', nominal: 5000000, keterangan: 'Donasi CSR', status: 'pending' },
        { id: 4, tanggal: '05/02/2026', kode: 'KM-004', kategori: 'SPP', sumber: 'Siswa (Batch)', nominal: 8750000, keterangan: 'Rekap SPP Minggu ke-1', status: 'verified' },
        { id: 5, tanggal: '01/02/2026', kode: 'KM-005', kategori: 'Hibah', sumber: 'Pemda Depok', nominal: 15000000, keterangan: 'Hibah operasional', status: 'verified' },
    ];

    const totalMasukBulanIni = kasMasukData.filter(d => d.status === 'verified').reduce((s, d) => s + d.nominal, 0);
    const totalPending = kasMasukData.filter(d => d.status === 'pending').reduce((s, d) => s + d.nominal, 0);
    ...
    return (
        <>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
                <div>
                    <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Kas Masuk (Penerimaan)</h2>
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">Pencatatan dan monitoring aliran dana masuk sekolah.</p>
                </div>
                <div className="flex flex-wrap gap-3">
                    <button className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm transition-all shadow-sm shadow-emerald-600/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col gap-4 relative overflow-hidden group hover:border-emerald-500/30 hover:shadow-md transition-all">
                    <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                        <span className="material-symbols-outlined text-8xl text-emerald-600">arrow_downward</span>
                    </div>
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 font-medium">Penerimaan Tervalidasi (Feb)</p>
                    </div>
                    <div className="relative z-10">
                        <h3 className="text-3xl font-extrabold text-slate-900 dark:text-white tabular-nums tracking-tight">Rp {formatNumber(totalMasukBulanIni)}</h3>
            ...
            <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-800 flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
                <div className="flex items-center gap-2 w-full md:w-auto">
                    <div className="relative w-full sm:w-64">
                        <input className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-emerald-500 transition-shadow" type="text" placeholder="Cari kode atau sumber..." />
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        <div className="relative w-full sm:w-40">
                            <select className="w-full pl-3 pr-8 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-emerald-500 appearance-none outline-none transition-shadow">
                        ...
                        <div className="relative w-full sm:w-40">
                            <input type="month" defaultValue="2026-02" className="w-full pl-3 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-emerald-500 appearance-none outline-none transition-shadow" />
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table className="w-full min-w-[1000px] border-collapse">
                        <thead>
                            <tr className="bg-slate-50/80 dark:bg-[#151724]/80 text-slate-500 dark:text-slate-400">
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider w-[150px]">Kategori</th>
                                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Sumber & Keterangan</th>
                                <th className="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider w-[180px]">Nominal Masuk</th>
                            ...
                            {kasMasukData.map((row) => (
                                <tr key={row.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold border ${row.kategori === 'Dana BOS' ? 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-900/20 dark:text-indigo-300 dark:border-indigo-800' :
                                            row.kategori === 'SPP' ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-blue-300 dark:border-blue-800' :
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{row.sumber}</p>
                                    ...
                                    <td className="px-6 py-4 text-sm font-bold text-right tabular-nums">
                                        <span className="text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1.5 rounded inline-block w-full text-right tracking-tight">
