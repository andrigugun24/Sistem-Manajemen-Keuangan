...
export default function DataGuruEdit({ guru }) {
    const { data, setData, processing, errors } = useForm({
        nama_guru: guru.nama_guru || '',
        nip: guru.nip || '',
        instansi: guru.instansi || 'SMP',
        foto: null,
    });

    const fotoInputRef = useRef(null);
    const [fotoPreview, setFotoPreview] = useState(null);
    const currentFoto = fotoPreview || (guru.foto ? `/storage/${guru.foto}` : null);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        router.post(route('guru.update', guru.id), {
            _method: 'put',
            ...data,
        }, { forceFormData: true });
    };
    ...
    return (
        <div className="space-y-6 lg:space-y-8 font-display antialiased pb-10">
            <Head title="Edit Data Guru / Staf" />
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mt-2">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h1 className="text-[28px] font-bold text-slate-900 dark:text-white tracking-tight">Edit Guru / Staf</h1>
                    </div>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm md:text-base ml-7">Perbarui identitas guru atau staf baru beserta instansi tempatnya mengajar.</p>
                </div>
            </div>
